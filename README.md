...
- **Note inside the reader**: an optional split within the reader pane, the note's text on top and the flashed word below. The current word carries a solid marker that moves as you read, with the sentence tinted around it. Drawn as the reader's own overlay, so it renders on every platform and iOS version. On by default on phones, where Obsidian cannot split the screen into two panes.
